...
class ComprehensiveEDA:
    ...
    def _print_guidance(self, analysis_name):
        """Enhanced interpretation guidance with ML decision focus"""
    ...
    def _describe_analysis(self, analysis_name):
        ...
        if analysis_name == 'missing_analysis':
            ...
            # Identify zero‑variance numeric
        ...
        # Add similar descriptive sections for other analyses...
        # For brevity, I'll show a few more examples:
            ...
            # Since this may be expensive, we compute it on the fly for the report
            try:
                X = self.df[self.num_cols + self.cat_cols].copy()
                y = self.df[self.target]
                for col in self.cat_cols:
        ...
        # Placeholder for other analyses – you can expand similarly
        else:
            # Generic fallback – note that analysis exists
    ...
    def _get_guidance_text(self, analysis_name):
        ...
        """
        # We can capture the output of _print_guidance by redirecting stdout,
        # but to keep it simple we'll store the guidance in a dictionary.
        # For brevity, I'll reuse the existing strings from _print_guidance.
        # (In practice you could call _print_guidance and capture the printed output,
        # but storing the strings directly is easier.)
        guidance = {
...
Missing data is often a hidden feature, not just a problem.""",
            ...
            'target_analysis': f"""🎯 TARGET ANALYSIS ({self.task.upper()})
...
If target is messy → model will ALWAYS struggle.""",
            ...
            # Add other guidance as needed – you can copy from _print_guidance.
        }
        # Fallback: if not found, return empty string
        return guidance.get(analysis_name, "")
