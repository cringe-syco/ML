...
class ComprehensiveEDA:
    ...
    # -------------------------------------------------------------------------
    # 31. Interpretation guidance printer
    # -------------------------------------------------------------------------
    def _print_guidance(self, analysis_name):
        guidance = {
            ...
            'statistical_validation': (
                ...
                "  - Normality: p>0.05 suggests normal distribution; non‑normal features may need transformation.\n"
                "  - ANOVA/t‑test: low p‑values indicate significant differences across categories.\n"
                "  - For regression, check linearity, homoscedasticity, and residual normality after model fitting."
            ),
            'leakage_detection': (
                "\n[Interpretation] Leakage detection:\n"
                "  - Features with |corr| > 0.95 with target likely contain target information (leakage).\n"
                "  - Unique identifiers should not be used as features unless they represent meaningful order."
            ),
            'feature_interactions': (
                "\n[Interpretation] Feature interactions:\n"
                "  - Interaction effects occur when the effect of one feature depends on another.\n"
                "  - Explore using product terms or tree‑based models (e.g., random forest)."
            ),
            'missing_as_signal': (
                "\n[Interpretation] Missing as signal:\n"
                "  - If missingness correlates with target, it can be a useful feature.\n"
                "  - Create missing indicators and include them in modelling."
            ),
            'drift_detection': (
                "\n[Interpretation] Drift detection:\n"
                "  - Low p‑values (<0.05) suggest feature distributions differ between train and test (or over time).\n"
                "  - Drift may degrade model performance; consider retraining or using drift‑aware techniques."
            ),
            'encoding_decisions': (
                "\n[Interpretation] Encoding decisions:\n"
                "  - Low cardinality (≤20): one‑hot encoding works well.\n"
                "  - High cardinality: target encoding or frequency encoding often performs better.\n"
                "  - For binary features, label encoding is sufficient."
            ),
            'categorical_interactions': (
                "\n[Interpretation] Categorical interactions:\n"
                "  - Pairs with high mutual information likely have strong joint predictive power.\n"
                "  - The heatmap shows the proportion of the majority class for each combination of categories.\n"
                "  - Consider creating these combined features for your model."
            ),
            'feature_combination_predictiveness': (
                "\n[Interpretation] Feature combination predictiveness:\n"
                "  - If adding combined features improves accuracy, your model benefits from interactions.\n"
                "  - Consider using feature hashing or interaction terms in your final model."
            ),
            'feature_stability_analysis': (
