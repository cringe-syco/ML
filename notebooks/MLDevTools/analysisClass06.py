...
class ComprehensiveEDA:
    ...
    AVAILABLE_ANALYSES = [
        ...
        'categorical_interactions',               # new
        'feature_combination_predictiveness',      # new   
        'feature_stability_analysis',
        'single_feature_performance',
        'monotonicity_analysis',
        'rare_category_analysis',
    ...
    def _get_feature_types(self):
        ...
        # Convert numeric columns to categorical based on variance OR cardinality
        for col in num[:]:
            # Skip columns with many missing values (>50%)
            ...
            if n_unique <= self.cat_card_threshold:
                ...
                continue  # already moved, skip variance check
    ...
    def categorical_interactions(self, top_n=10, max_categories_per_feature=20):
        ...
        top_n : int, default=10
            Number of top feature pairs to display.
        max_categories_per_feature : int, default=20
            If a categorical feature has more unique values than this, it is skipped to avoid explosion.
        """
        if self.task == 'regression':
        ...
        # Filter features with manageable cardinality
        ...
        for i in range(len(usable_cats)):
            for j in range(i+1, len(usable_cats)):
                ...
                # Create combined feature: concatenation of both columns
        ...
        # Sort by MI descending and keep top_n
        ...
        # For the best pair, create a detailed heatmap
        ...
        # Create a combined feature for the top pair
        combo = self.df[col1].astype(str) + ' | ' + self.df[col2].astype(str)

        # Get target distribution for each combination (normalized by row)
        target_dist = pd.crosstab(combo, self.df[self.target], normalize='index')

        # For each combination, find the proportion of the most frequent class
        max_prop = target_dist.max(axis=1)

        # Build a pivot table: rows = col1, columns = col2, values = max_prop
        # We need to split the combo index back into col1 and col2
        index_df = target_dist.index.to_series().str.split(' | ', expand=True)
        index_df.columns = [col1, col2]

        # Combine with max_prop values
    ...
    # -------------------------------------------------------------------------
    # 23. Feature combination predictiviness
    # -------------------------------------------------------------------------
    def feature_combination_predictiveness(self, max_combinations=50, test_size=0.2):
        """
        Evaluate how much predictive power is gained by combining categorical features.
        
        Parameters:
        max_combinations : int, default=50
            Maximum number of pairs to evaluate (to limit computation).
        ...
        # Prepare base features (numerical + categorical, encode categorical)
        X_base = self.df[self.num_cols + self.cat_cols].copy()
        # Encode categorical features for the base model
        from sklearn.preprocessing import LabelEncoder
        for col in self.cat_cols:
            X_base[col] = LabelEncoder().fit_transform(X_base[col].astype(str))
        X_base = X_base.fillna(X_base.median())  # simple imputation
        ...
        # Create new features by combining pairs of categorical columns
        # Use hashing to keep dimensionality manageable
        hasher = FeatureHasher(n_features=100, input_type='string')
        X_combined = X_base.copy()
        # For each pair, add hashed combination
        # We'll only consider a subset of pairs to keep runtime reasonable
        import itertools
        pairs = list(itertools.combinations(self.cat_cols, 2))
        if len(pairs) > max_combinations:
            pairs = pairs[:max_combinations]

        for col1, col2 in pairs:
            combo = (self.df[col1].astype(str) + '_' + self.df[col2].astype(str)).values.reshape(-1,1)
            hashed = hasher.transform(combo).toarray()
            # Add as new columns (but we need to align with X_train)
            # For simplicity, we'll just retrain a new model with all combinations
            # This might be slow if many pairs. Alternatively, we can add them incrementally.
            # We'll add them all at once after encoding
            pass

        # A simpler approach: add a new feature that is the hash of the concatenation of all categorical columns
    ...
    def redundancy_pruning(self, corr_threshold=0.85):
        """
        Identify groups of highly correlated numeric features and suggest pruning.
        Returns a list of features to drop (one per group) based on highest correlation with target.
        """
    ...
    def feature_stability_analysis(self, test_df=None, bootstrap_n=0):
        """
        Assess feature stability between train and test sets (if provided) or via bootstrap.
        
        Parameters:
        test_df : DataFrame, optional
            Separate test dataset to compare against training.
        bootstrap_n : int, default=0
            If >0, number of bootstrap samples to estimate stability (variance of feature means).
        """
    ...
    def single_feature_performance(self, test_size=0.2, max_features=None):
        """
        Evaluate predictive power of each feature individually using a simple model.
        
        Parameters:
        test_size : float, default=0.2
            Fraction of data to hold out for evaluation.
        max_features : int, optional
            Limit to top N features (by variance or correlation) to speed up.
        """
    ...
    def monotonicity_analysis(self):
        """
        For each numeric feature, test monotonic relationship with target using Spearman rank correlation.
        For classification, also check if the odds ratio changes monotonically (by numeric encoding of target).
        """
    ...
    def rare_category_analysis(self, min_freq=0.01):
        """
        For each categorical feature, identify rare categories (frequency < min_freq) and
        show their impact on the target (mean target for regression, class distribution for classification).
        """
    ...
    def temporal_leakage_check(self, time_col=None):
        """
        Identify features that may cause temporal leakage: either directly derived from time
        or perfectly correlated with time (if time_col provided).
        """
    ...
    def feature_recommendation_engine(self, test_df=None, time_col=None):
        """
        Generate a comprehensive recommendation for each feature based on:
        - Stability (train/test p-value)
        - Single feature performance
        - Monotonicity (Spearman correlation)
        - Redundancy (suggested drop)
        - Rare categories (warning for categorical)
        - Temporal leakage (warning)
        Returns a DataFrame with recommendations.
        """
        logging.info("===== FEATURE RECOMMENDATION ENGINE =====")
        # Run lightweight analyses (others are optional)
    ...
    def _print_guidance(self, analysis_name):
        ...
        guidance = {
            ...
            'feature_stability_analysis': (
                "\n[Interpretation] Feature stability:\n"
                "  - Low p‑values (<0.05) in KS/chi² tests indicate different distributions between train and test.\n"
                "  - Unstable features may reduce model generalizability."
            ),
            'single_feature_performance': (
                "\n[Interpretation] Single‑feature performance:\n"
                "  - Features with high scores are strong predictors individually.\n"
                "  - Low scores don't mean useless – they may be useful in combination with others."
            ),
            'monotonicity_analysis': (
                "\n[Interpretation] Monotonicity:\n"
                "  - Spearman correlation |r|>0.5 suggests a monotonic relationship.\n"
                "  - Monotonic features are easier to interpret and often work well with linear models."
            ),
            'rare_category_analysis': (
                "\n[Interpretation] Rare categories:\n"
                "  - Rare categories can cause overfitting or instability.\n"
                "  - Consider grouping them into an 'Other' category or removing them."
            ),
            'redundancy_pruning': (
                "\n[Interpretation] Redundancy pruning:\n"
                "  - Highly correlated features provide little additional information.\n"
                "  - Removing redundant features can simplify the model without losing predictive power."
            ),
            'temporal_leakage_check': (
                "\n[Interpretation] Temporal leakage:\n"
                "  - Features that correlate strongly with time may leak future information.\n"
                "  - Ensure such features are not used in models that predict future outcomes."
            ),
            'feature_recommendation_engine': (
                "\n[Interpretation] Feature recommendation:\n"
                "  - The engine compiles insights to suggest which features to keep, transform, or drop.\n"
                "  - Use this as a starting point for feature selection."
            )
